...
#define COUNT 10
 /**
  * Cây AVL là một cấu trúc dữ liệu thuộc họ "Cây tìm kiếm nhị phân tự cân bằng". Đặc điểm nổi trội nhất của nó so với các cấu trúc dữ liệu 
  * thông thường như mảng tuyến tính và cây nhị phân tìm kiếm thồng thường là độ phức tạp của 3 phép toán cơ bản thao tác trên cây 
  * là: Chèn, xóa và tìm kiếm đều là O(log n).

Vậy nên một trong những ứng dụng phổ biến của cây AVL là dùng để lưu trữ thông tin cần truy xuất nhanh: Từ điển (hay dùng nhất), 
cấu trúc dữ liệu dạng văn bản (cây văn bản) như Open Office (trong cơ sở dữ liệu của Open Office) ... Rất nhiều
 */
using namespace std;
